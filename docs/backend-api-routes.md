...
| GET | `/allocation/cycles` | `app.api.routes.allocation.list_allocation_cycles` | `PASSIVE_READ` | `list[dict[str, object]]` | `getAllocationCycles` | Allocation read model. Needs modeled schema. |
| GET | `/allocation/cycles/{cycle_id}` | `app.api.routes.allocation.get_allocation_cycle` | `PASSIVE_READ` | `dict[str, object]` | `getAllocationCycle` | Allocation read model. Needs modeled schema. |
| GET | `/allocation/intents` | `app.api.routes.allocation.list_allocation_intents` | `PASSIVE_READ` | `list[dict[str, object]]` | `getAllocationIntents` | Allocation read model. Needs modeled schema. |
| GET | `/allocation/intents/{trade_intent_id}` | `app.api.routes.allocation.get_allocation_intent` | `PASSIVE_READ` | `dict[str, object]` | `getAllocationIntent` | Allocation read model. Needs modeled schema. |
| GET | `/allocation/drift` | `app.api.routes.allocation.get_allocation_drift_summary` | `PASSIVE_READ` | `dict[str, object]` | `getAllocationDriftSummary` | Computed allocation drift read model. Needs modeled schema. |
| GET | `/allocation/alerts` | `app.api.routes.allocation.list_allocation_alerts` | `ACTIVE_READ_REFRESH` by default | `list[dict[str, object]]` | `getAllocationAlerts` | Default `refresh=true` can persist alert rows. `refresh=false` is the passive variant. See `AUDIT-003`. |
| POST | `/allocation/alerts/{alert_id}/acknowledge` | `app.api.routes.allocation.acknowledge_allocation_alert` | `MUTATION` | `dict[str, object]` | `acknowledgeAllocationAlert` | Acknowledges alert state. Needs modeled schema. |
| POST | `/allocation/alerts/{alert_id}/resolve` | `app.api.routes.allocation.resolve_allocation_alert` | `MUTATION` | `dict[str, object]` | `resolveAllocationAlert` | Resolves alert state. Needs modeled schema. |
| GET | `/allocation/alerts/unresolved-critical` | `app.api.routes.allocation.list_unresolved_critical_allocation_alerts` | `ACTIVE_READ_REFRESH` | `list[dict[str, object]]` | `getUnresolvedCriticalAllocationAlerts` | Hard-codes alert refresh. See `AUDIT-003`. |
| GET | `/allocation/exposure` | `app.api.routes.allocation.get_allocation_exposure_summary` | `PASSIVE_READ` | `dict[str, object]` | `getAllocationExposureSummary` | Computed exposure read model. Needs modeled schema. |
...
| GET | `/markets/overview` | `app.api.routes.markets.get_market_overview` | `BROKER_READ` | `dict[str, object]` | `getMarketOverview` | Calls broker market-details read path. Needs modeled schema. |
| GET | `/markets/catalogue` | `app.api.routes.markets.get_market_catalogue` | `PASSIVE_READ` | `dict[str, object]` | `getMarketCatalogue` | Reads catalogue/runtime projection. Needs modeled schema. |
| GET | `/watchlist/shortlist` | `app.api.routes.markets.get_shortlist` | `PASSIVE_READ` | `dict[str, object]` | None found | Reads shortlist projection. Needs modeled schema. |
| POST | `/watchlist/shortlist/{instrument_id}` | `app.api.routes.markets.add_shortlist_item` | `MUTATION` | `dict[str, object]` | `addShortlistInstrument` | Adds or updates shortlist entry. Needs modeled schema. |
| DELETE | `/watchlist/shortlist/{instrument_id}` | `app.api.routes.markets.remove_shortlist_item` | `MUTATION` | `dict[str, object]` | `removeShortlistInstrument` | Removes or deactivates shortlist entry. Needs modeled schema. |
| POST | `/strategy-watchlist/bulk` | `app.api.routes.markets.add_strategy_watchlist_items` | `MUTATION` | `dict[str, object]` | `addStrategyWatchlistInstruments` | Adds strategy-watchlist entries and can sync system entries. Needs modeled schema. |
| GET | `/strategy-watchlist` | `app.api.routes.markets.get_strategy_watchlist` | `ACTIVE_READ_REFRESH` | `dict[str, object]` | `getStrategyWatchlist` | Can sync system watchlist entries. Needs modeled schema. See `AUDIT-API-002`. |
| DELETE | `/strategy-watchlist/{instrument_id}` | `app.api.routes.markets.remove_strategy_watchlist_item` | `MUTATION` | `dict[str, object]` | `removeStrategyWatchlistInstrument` | Removes or cools down strategy-watchlist entry. Needs modeled schema. |
| GET | `/market-data/feed-state` | `app.api.routes.markets.get_feed_state` | `ACTIVE_READ_REFRESH` | `dict[str, object]` | `getFeedState` | Can sync watchlist state and call broker market reads. Needs modeled schema. See `AUDIT-API-002`. |
| GET | `/market-data/feed-state/{instrument_id}` | `app.api.routes.markets.get_instrument_feed_state` | `BROKER_READ` | `dict[str, object]` | `getInstrumentFeedState` | Calls broker market-status read path. Needs modeled schema. |
| GET | `/live/instruments/{instrument_id}/chart` | `app.api.routes.markets.get_live_instrument_chart` | `BROKER_READ` | `dict[str, object]` | `getLiveInstrumentChart` | Can call broker historical-candle and market-status reads. Needs modeled schema. |
...
| GET | `/aimee/snapshot` | `app.api.routes.aimee.get_snapshot` | `BROKER_READ` | `dict[str, object]` | `getAimeeSnapshot` | Avoids review persistence, but indirectly calls dashboard broker account read. Needs modeled schema. See `AUDIT-API-007`. |
| GET | `/reviews/operator-summary` | `app.api.routes.ai_reviewer.get_operator_summary` | `ACTIVE_READ_REFRESH` | `OperatorSummaryReview` | `getOperatorSummaryReview` | Persists generated review record by default and can call dashboard broker read. See `AUDIT-API-003`. |
| GET | `/reviews/daily` | `app.api.routes.ai_reviewer.get_daily_review` | `ACTIVE_READ_REFRESH` | `DailyReviewResponse` | None found | Persists generated review record by default. See `AUDIT-API-003`. |
| GET | `/reviews/strategies/{strategy_name}` | `app.api.routes.ai_reviewer.get_strategy_review` | `ACTIVE_READ_REFRESH` | `StrategyReviewResponse` | None found | Persists generated review record by default. See `AUDIT-API-003`. |
| GET | `/reviews/runtime-health` | `app.api.routes.ai_reviewer.get_runtime_health_review` | `ACTIVE_READ_REFRESH` | `RuntimeHealthReviewResponse` | None found | Persists generated review record by default. See `AUDIT-API-003`. |
| GET | `/reviews/trades/{trade_id}/postmortem` | `app.api.routes.ai_reviewer.get_trade_postmortem` | `ACTIVE_READ_REFRESH` | `TradePostMortemReviewResponse` | None found | Persists generated review record by default. See `AUDIT-API-003`. |
