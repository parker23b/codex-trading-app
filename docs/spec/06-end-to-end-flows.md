...
| API routes | `GET /allocation/cycles` — `PASSIVE_READ`; `GET /allocation/cycles/{cycle_id}` — `PASSIVE_READ`; `GET /allocation/intents` — `PASSIVE_READ`; `GET /allocation/intents/{trade_intent_id}` — `PASSIVE_READ`; `GET /allocation/drift` — `PASSIVE_READ`; `GET /allocation/alerts` — `PASSIVE_READ` by default; `GET /allocation/alerts?refresh=true` — `ACTIVE_READ_REFRESH`; `POST /allocation/alerts/{alert_id}/acknowledge` — `MUTATION`; `POST /allocation/alerts/{alert_id}/resolve` — `MUTATION`; `GET /allocation/alerts/unresolved-critical` — `PASSIVE_READ`; `GET /allocation/exposure` — `PASSIVE_READ`. |
