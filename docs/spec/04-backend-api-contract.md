...
| GET | `/markets/catalogue` | `routes/markets.py:get_market_catalogue` | `PASSIVE_READ` | Catalogue projection; watchlist sync audit needed | Markets page | Raw dict needs schema | Medium |
| GET | `/watchlist/shortlist` | `routes/markets.py:get_shortlist` | `PASSIVE_READ` | Watchlist read | Markets page | Raw dict needs schema | Medium |
...
| GET | `/charts/risk-allocation` | `routes/charts.py:get_risk_allocation_chart` | `PASSIVE_READ` | Chart projection | Needs confirmation | Raw dict needs schema | Medium |
| GET | `/positions` | `routes/positions.py:list_positions` | `PASSIVE_READ` | Local positions read; compatibility status needs audit | Needs confirmation/compat | Pydantic list model | High |
