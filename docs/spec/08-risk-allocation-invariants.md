...
For one account/risk book, the aggregate portfolio snapshot, allocation decision, hard-risk checks, and durable active-intent admission form one serialized critical section. In-memory locking is sufficient only for explicitly single-process local operation. Multi-worker Postgres operation requires a database-coordinated lock or equivalent atomic reservation mechanism.
...
| RISK-014 | Concurrent or repeated allocation cycles MUST NOT admit duplicate/conflicting exposure, exceed budgets through race conditions, or bypass active intent uniqueness. The lock/reservation boundary must cover aggregate snapshot calculation through durable intent admission and must coordinate across worker processes. | Tests for simultaneous candidates, same-instrument conflicts, duplicate direction conflicts, overlapping cycles, active intent uniqueness, and separate Postgres connections contending for one risk book. | P0 | High |
