...
def test_tier2_refresh_plan_uses_capped_and_seed_candidates(session):
    ...
    service.settings.ig_streaming_seed_instruments = ["CS.D.EURUSD.CFD.IP", "IX.D.SP500.DAILY.IP"]
...
def test_record_tier2_refresh_updates_timestamp(session):
    ...
    service.record_tier2_refresh(instrument="COM.D.XAUUSD.CFD.IP", refreshed_at=refreshed_at)
