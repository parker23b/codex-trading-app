...
def _fingerprint_state(session) -> dict[str, object]:
    return {
        "reviews": [
            ...
            for record in session.exec(select(GeneratedReviewRecord).order_by(GeneratedReviewRecord.id)).all()
        ],
        "reconciliation_events": [
            (event.id, event.event_type, event.strategy_name, event.instrument, event.created_at.isoformat())
            for event in session.exec(select(ReconciliationEvent).order_by(ReconciliationEvent.id)).all()
        ],
        "domain_events": [
            (event.id, event.event_type, event.category, event.severity, event.created_at.isoformat())
            for event in session.exec(select(DomainEvent).order_by(DomainEvent.id)).all()
        ],
        "operator_control": [
            (row.id, row.autonomous_control_override, row.override_reason, row.updated_at.isoformat())
            for row in session.exec(select(OperatorControlState).order_by(OperatorControlState.id)).all()
        ],
        "governance": [
            (row.strategy_name, row.approval_state, row.autonomous_operation_allowed, row.emergency_stop, row.updated_at.isoformat())
            for row in session.exec(select(StrategyFamilyGovernance).order_by(StrategyFamilyGovernance.strategy_name)).all()
        ],
        "deployments": [
            (row.strategy_name, row.state, row.selected_instrument, row.selected_profile, row.updated_at.isoformat())
            for row in session.exec(select(StrategyDeployment).order_by(StrategyDeployment.strategy_name)).all()
        ],
        "runtimes": [
            (row.runtime_id, row.strategy_name, row.instrument, row.status, row.recovery_state, row.control_mode, row.updated_at.isoformat())
            for row in session.exec(select(StrategyRuntimeState).order_by(StrategyRuntimeState.runtime_id)).all()
        ],
        "watchlist": [
            (
                row.instrument,
                ...
                row.status,
                row.pinned,
                row.last_streamed_at.isoformat() if row.last_streamed_at is not None else None,
                row.last_refreshed_at.isoformat() if row.last_refreshed_at is not None else None,
                row.updated_at.isoformat(),
            )
            for row in session.exec(select(WatchlistEntry).order_by(WatchlistEntry.instrument)).all()
        ],
        "executions": [
            (row.id, row.strategy_name, row.instrument, row.status, row.requires_manual_review, row.last_transition_at.isoformat(), row.updated_at.isoformat())
...
def test_aimee_snapshot_route_is_side_effect_free(session, fixed_now, monkeypatch):
    ...
    monkeypatch.setattr("app.services.broker_service.BrokerService.reconcile_positions", _fail_if_called("BrokerService.reconcile_positions"))
    monkeypatch.setattr("app.services.reconciliation_service.ReconciliationService.reconcile_open_positions", _fail_if_called("ReconciliationService.reconcile_open_positions"))
    monkeypatch.setattr("app.services.strategy_governance_service.StrategyGovernanceService.ensure_defaults", _fail_if_called("StrategyGovernanceService.ensure_defaults"))
    monkeypatch.setattr("app.services.watchlist_service.WatchlistService.get_streaming_plan", _fail_if_called("WatchlistService.get_streaming_plan"))
    monkeypatch.setattr("app.services.watchlist_service.WatchlistService.get_tier2_refresh_plan", _fail_if_called("WatchlistService.get_tier2_refresh_plan"))
...
def test_operational_question_persists_only_the_explicit_question_review(session, fixed_now, monkeypatch):
    _seed_read_models(session, fixed_now)
    reviewer = AIReviewerService(session)
    before = _fingerprint_state(session)
    original_persist_review = AIReviewerService._persist_review
    ...
    response = reviewer.answer_operational_question("What needs my attention right now?")
