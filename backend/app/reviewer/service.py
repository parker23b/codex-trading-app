...
class AIReviewerService:
    ...
    def get_operator_summary(self, *, persist: bool = True) -> OperatorSummaryReview:
        ...
        rejected_intents = self._list_rejected_trade_intents(
            start=since, end=now, limit=500
        )
        ...
        ranked_positions = sorted(
            positions, key=lambda position: position.risk_percent or 0.0, reverse=True
        )
        top_risk_exposures = [
            ExposureFact(
                ...
                notional_estimate=round(
                    (position.current_price or position.open_price) * position.size, 2
                ),
                share_of_open_risk_percent=round(
                    ((position.risk_percent or 0.0) / total_open_risk) * 100, 2
                )
                if total_open_risk
                else 0.0,
            )
        ...
        baseline_largest_share = self._previous_fact(
            previous, "largest_risk_share_percent"
        )
        baseline_trade_count = self._rolling_trade_count_baseline(
            now, windows=5, window_days=1
        )
        baseline_win_rate = self._rolling_win_rate_baseline(
            now, windows=5, window_days=1
        )
        strategy_trade_groups = self._group_trades_by_strategy(trades)
        largest_share = (
            top_risk_exposures[0].share_of_open_risk_percent
            if top_risk_exposures
            else 0.0
        )

        facts = OperatorSummaryFacts(
            account_value=self._float_or_none(dashboard.get("accountValue")),
            account_value_change_percent=self._float_or_none(
                dashboard.get("accountValuePercent")
            ),
            ...
            strategy_health=[
                StrategyHealthFact(
                    ...
                    active_runtime_count=len(
                        [
                            runtime
                            for runtime in runtimes
                            if runtime.strategy_name == strategy_name
                            and runtime.status == "RUNNING"
                        ]
                    ),
                ...
                for strategy_name, strategy_trades in sorted(
                    strategy_trade_groups.items()
                )
            ],
            risk_rejections_24h=len(rejected_intents),
            execution_failures_24h=len(
                [
                    execution
                    for execution in executions
                    if self._is_since(execution.last_transition_at, since)
        ...
        supporting_metrics = [
            ...
            self._metric(
                "open_risk_percent",
                "Open Risk",
                facts.open_risk_percent,
                unit="pct",
                baseline=facts.baseline_open_risk_percent,
            ),
            self._metric(
                "largest_risk_share_percent",
                "Largest Risk Share",
                facts.largest_risk_share_percent,
                unit="pct",
                baseline=facts.baseline_largest_risk_share_percent,
            ),
            self._metric(
                "trade_count_24h",
                "Trade Count (24h)",
                len(trades),
                baseline=facts.baseline_trade_count_24h,
            ),
            self._metric(
                "win_rate_24h",
                "Win Rate (24h)",
                ...
                unit="pct",
                baseline=facts.baseline_win_rate_24h,
            ),
            self._metric("stale_runtimes", "Stale Runtimes", facts.stale_runtimes),
        ]
        derived_observations = self._operator_observations(
            facts, trades, executions, recon_events, stale_runtimes, now
        )
        possible_contributors = self._operator_contributors(
            facts, executions, rejected_intents, stale_runtimes, stream_health
        )
        warnings = self._default_warnings(
            stream_health=stream_health, stale_runtimes=len(stale_runtimes)
        )

        response = OperatorSummaryReview(
            metadata=self._metadata(
                "operator_summary", now, since, now, {"window": "24h"}
            ),
            facts=facts,
    ...
    def get_daily_review(
        self, review_date: date, *, persist: bool = True
    ) -> DailyReviewResponse:
        ...
        baseline_trades = self.trade_service.list_trades(
            date_from=previous_start, date_to=previous_end
        )
        positions = self.trade_service.list_positions()
        executions = self.trade_service.list_executions(limit=500)
        risk_rejections = self._list_rejected_trade_intents(
            start=start, end=end, limit=500
        )
        runtimes = self.runtime_state_service.list_runtimes()
        recon_events = self.trade_service.list_reconciliation_events(
            date_from=start, date_to=end
        )
        failures = [
            execution
            for execution in executions
            if self._in_period(execution.last_transition_at, start, end)
            and execution.status
            in {ExecutionStatus.FAILED.value, ExecutionStatus.NEEDS_MANUAL_REVIEW.value}
        ]
        runtime_issues = [
            runtime
            for runtime in self._stale_runtimes(runtimes, end)
            if self._in_period(runtime.updated_at, start, end)
        ]
        facts = DailyReviewFacts(
            review_date=review_date,
            strategies_ran=sorted(
                {trade.strategy_name for trade in trades}
                | {
            ...
            active_instruments=sorted(
                {trade.instrument for trade in trades}
                | {
                    runtime.instrument
                    for runtime in runtimes
                    if self._in_period(runtime.updated_at, start, end)
            ...
            unrealised_pnl=round(
                sum(position.unrealized_pnl or 0.0 for position in positions), 2
            ),
            ...
            baseline_trade_count=round(len(baseline_trades) / 5, 2)
            if baseline_trades
            else None,
            baseline_realised_pnl=round(
                sum(trade.pnl for trade in baseline_trades) / 5, 2
            )
            if baseline_trades
            else None,
        ...
        response = DailyReviewResponse(
            ...
            facts=facts,
            derived_observations=self._daily_observations(
                facts, failures, recon_events
            ),
            possible_contributors=self._daily_contributors(
                facts, failures, risk_rejections, runtime_issues
            ),
            warnings=self._default_warnings(
                stream_health=self.stream_service.get_health(),
                stale_runtimes=len(runtime_issues),
            ),
            supporting_metrics=[
                self._metric(
                    "trade_count",
                    "Trades",
                    facts.trade_count,
                    baseline=facts.baseline_trade_count,
                ),
                self._metric(
                    "realised_pnl",
                    "Realised PnL",
                    facts.realised_pnl,
                    unit="ccy",
                    baseline=facts.baseline_realised_pnl,
                ),
                self._metric(
                    "win_rate",
                    "Win Rate",
                    self._win_rate(trades),
                    unit="pct",
                    baseline=facts.baseline_win_rate,
                ),
                self._metric(
                    "risk_rejections", "Risk Rejections", facts.risk_rejections
                ),
            ],
        )
        return self._finalize_review(response, persist=persist)
    ...
    def get_strategy_review(
        self, strategy_name: str, period_days: int = 7, *, persist: bool = True
    ) -> StrategyReviewResponse:
        now = self._utc_datetime(datetime.now(UTC))
        ...
        trades = [
            ...
            for trade in self.trade_service.list_trades(
                strategy_name=strategy_name, date_from=previous_start
            )
            if trade.strategy_name == strategy_name
        ]
        current_trades = [
            trade for trade in trades if self._utc_datetime(trade.close_time) >= start
        ]
        previous_trades = [
            trade
            for trade in trades
            if previous_start <= self._utc_datetime(trade.close_time) < previous_end
        ]
        positions = [
            position
        ...
        executions = [
            ...
            if execution.strategy_name == strategy_name
            and self._is_since(execution.last_transition_at, start)
        ]
        risk_rejections = self._list_rejected_trade_intents(
            start=start, end=now, strategy_name=strategy_name, limit=500
        )
        runtimes = [
            runtime
        ...
        facts = StrategyReviewFacts(
            ...
            status="RUNNING"
            if any(runtime.status == "RUNNING" for runtime in runtimes)
            else "STOPPED",
            active_runtime_count=len(
                [runtime for runtime in runtimes if runtime.status == "RUNNING"]
            ),
            ...
            baseline_trade_count=float(len(previous_trades))
            if previous_trades
            else None,
        ...
        response = StrategyReviewResponse(
            metadata=self._metadata(
                ...
                start,
                now,
                {"strategy_name": strategy_name, "period_days": period_days},
            ),
            facts=facts,
            derived_observations=self._strategy_observations(facts),
            possible_contributors=self._strategy_contributors(
                facts, executions, risk_rejections, stale_runtimes
            ),
            warnings=self._default_warnings(
                stream_health=self.stream_service.get_health(),
                stale_runtimes=len(stale_runtimes),
            ...
            supporting_metrics=[
                ...
                self._metric(
                    "realised_pnl", "Realised PnL", facts.realised_pnl, unit="ccy"
                ),
                self._metric(
                    "execution_failures", "Execution Failures", facts.execution_failures
                ),
            ],
        )
        return self._finalize_review(response, persist=persist)
    ...
    def get_runtime_health_review(
        self, period_hours: int = 24, *, persist: bool = True
    ) -> RuntimeHealthReviewResponse:
        now = self._utc_datetime(datetime.now(UTC))
        ...
        executions = [
            execution
            for execution in self.trade_service.list_executions(limit=500)
            if self._is_since(execution.last_transition_at, start)
        ]
        risk_rejections = self._list_rejected_trade_intents(
            start=start, end=now, limit=500
        )
        stream_health = self.stream_service.get_health()
        stale_runtimes = self._stale_runtimes(runtimes, now)
        heartbeat_issues = [
            ...
            and (
                ...
                or (now - self._utc_datetime(runtime.last_heartbeat_at)).total_seconds()
                > 90
            )
        ]
        issues = [
            RuntimeIssueFact(
                ...
                last_seen_at=self._utc_datetime(runtime.last_price_seen_at)
                if runtime.last_price_seen_at is not None
                else None,
            )
            for runtime in stale_runtimes
        ] + [
            RuntimeIssueFact(
                ...
                last_seen_at=self._utc_datetime(runtime.last_heartbeat_at)
        ...
        facts = RuntimeHealthFacts(
            ...
            polling_fallback_suspected=stream_health.enabled
            and not stream_health.connected
            and len(stale_runtimes) == 0,
        ...
        response = RuntimeHealthReviewResponse(
            metadata=self._metadata(
                "runtime_health_review", now, start, now, {"period_hours": period_hours}
            ),
            facts=facts,
            ...
            supporting_metrics=[
                self._metric(
                    "stale_price_count", "Stale Prices", facts.stale_price_count
                ),
                self._metric(
                    "heartbeat_issue_count",
                    "Heartbeat Gaps",
                    facts.heartbeat_issue_count,
                ),
                self._metric(
                    "reconciliation_issue_count",
                    "Reconciliation Issues",
                    facts.reconciliation_issue_count,
                ),
                self._metric(
                    "execution_failure_count",
                    "Execution Failures",
                    facts.execution_failure_count,
                ),
            ],
        )
        return self._finalize_review(response, persist=persist)
    ...
    def get_trade_postmortem(
        ...
        instrument_losses = [
            ...
            if item.instrument == trade.instrument and item.pnl < 0
        ]
        ...
        executions = [
            ...
            <= self._utc_datetime(execution.last_transition_at)
        ...
        response = TradePostMortemReviewResponse(
            metadata=self._metadata(
                "trade_postmortem",
                self._utc_datetime(datetime.now(UTC)),
                open_time,
                close_time,
                {"trade_id": trade_id},
            ),
            ...
            possible_contributors=self._postmortem_contributors(
                trade, executions, strategy_losses
            ),
            warnings=self._default_warnings(
                stream_health=self.stream_service.get_health(), stale_runtimes=0
            ),
            supporting_metrics=[
                self._metric("pnl", "PnL", facts.pnl, unit="ccy"),
                self._metric(
                    "holding_minutes", "Holding Time", facts.holding_minutes, unit="min"
                ),
                self._metric(
                    "recent_loss_count_same_strategy",
                    "Recent Strategy Losses",
                    facts.recent_loss_count_same_strategy,
                ),
            ],
        )
        return self._finalize_review(response, persist=persist)
    ...
    def answer_operational_question(
        self, question: str, strategy_name: str | None = None
    ) -> OperationalQuestionReviewResponse:
        ...
        response = OperationalQuestionReviewResponse(
            metadata=self._metadata(
                ...
                self._utc_datetime(datetime.now(UTC)),
                supporting.metadata.period_start,
                supporting.metadata.period_end,
                {"question": question, "strategy_name": inferred_strategy_name},
            ),
    ...
    def list_review_history(
        self, review_type: str | None = None, limit: int = 20
        ...
        if review_type:
            statement = statement.where(
                GeneratedReviewRecord.review_type == review_type
            )
    ...
    def _finalize_review(
        self, response: Any, request_text: str | None = None, *, persist: bool = True
    ) -> Any:
        facts_payload = (
            response.facts.model_dump(mode="json")
            if hasattr(response.facts, "model_dump")
            else response.facts
        )
        review_payload = {
            "facts": facts_payload,
            "derived_observations": [
                item.model_dump(mode="json") for item in response.derived_observations
            ],
            "possible_contributors": [
                item.model_dump(mode="json") for item in response.possible_contributors
            ],
            "warnings": [item.model_dump(mode="json") for item in response.warnings],
            "supporting_metrics": [
                item.model_dump(mode="json") for item in response.supporting_metrics
            ],
            "metadata": {
        ...
        system_prompt, user_prompt = build_review_prompts(
            response.metadata.review_type, review_payload, request_text=request_text
        )
        llm_response = self.llm_client.generate(
            ReviewLLMRequest(system_prompt=system_prompt, user_prompt=user_prompt)
        )
        response.metadata.generation_mode = (
    ...
    def _persist_review(self, response: Any) -> None:
        record = GeneratedReviewRecord(
            ...
            ai_summary=response.ai_summary.model_dump(mode="json")
            if response.ai_summary is not None
            else None,
            prompt_version=response.provenance.prompt_version
            if response.provenance is not None
            else PROMPT_VERSION,
            provider=response.provenance.llm_provider
            if response.provenance is not None
            else None,
            model=response.provenance.llm_model
            if response.provenance is not None
            else None,
            raw_model_response=response.provenance.raw_response
            if response.provenance is not None
            else None,
    ...
    def _metadata(
        ...
        *,
        ...
        return ReviewMetadata(
            ...
            period_start=self._utc_datetime(period_start)
            if period_start is not None
            else None,
            period_end=self._utc_datetime(period_end)
    ...
    def _coverage_notes(self) -> list[str]:
        notes = [
            "Reviews are grounded in persisted trade intents, trades, open positions, execution attempts, runtime state, reconciliation events, and stream health."
        ]
        if not self.settings.ai_reviewer_llm_enabled:
            notes.append(
                "LLM explanation is disabled; reviews remain deterministic and audit-ready."
            )
        return notes

    def _default_warnings(
        self, *, stream_health: Any, stale_runtimes: int
    ) -> list[ReviewWarning]:
        ...
        if not self.settings.ai_reviewer_llm_enabled:
            warnings.append(
                ReviewWarning(
                    code="llm_disabled",
                    severity="info",
                    message="AI explanation is disabled; deterministic summaries remain authoritative.",
                )
            )
        if stream_health.enabled and not stream_health.connected:
            warnings.append(
                ReviewWarning(
                    ...
                    severity="warning",
                    message="Streaming is enabled but disconnected, so freshness and execution context may rely on polled or cached data.",
                )
            )
        if stale_runtimes > 0:
            warnings.append(
                ReviewWarning(
                    code="price_freshness_degraded",
                    severity="warning",
                    message=f"{stale_runtimes} runtime(s) have stale price timestamps, which weakens some operational interpretations.",
                )
            )
        return warnings
    ...
    def _operator_observations(
        ...
        risk_delta = self._delta_value(
            facts.open_risk_percent, facts.baseline_open_risk_percent
        )
        largest_share_delta = self._delta_value(
            facts.largest_risk_share_percent, facts.baseline_largest_risk_share_percent
        )
        trade_count_delta = self._delta_value(
            float(len(trades)), facts.baseline_trade_count_24h
        )
        win_rate_delta = self._delta_value(
            self._win_rate(trades), facts.baseline_win_rate_24h
        )
        if facts.main_open_risk is not None:
            observations.append(
                self._obs(
                    ...
                    severity="critical"
                    if facts.largest_risk_share_percent >= 50
                    or (largest_share_delta is not None and largest_share_delta >= 15)
                    else "warning",
        ...
        if stale_runtimes:
            stale_severity = (
                "critical"
                if len(stale_runtimes) >= max(1, facts.active_runtimes // 2)
                else "warning"
            )
            observations.append(
                self._obs(
                    ...
                    metrics=[
                        self._obs_metric(
                            "stale_runtimes", "Stale Runtimes", len(stale_runtimes)
                        ),
                        self._obs_metric(
                            "active_runtimes", "Active Runtimes", facts.active_runtimes
                        ),
                    ],
                )
            )
        ...
        if recon_patterns:
            top_pattern, top_count = sorted(
                recon_patterns.items(), key=lambda item: item[1], reverse=True
            )[0]
            observations.append(
                self._obs(
                    ...
                    metrics=[
                        self._obs_metric(
                            "reconciliation_issues_24h",
                            "Reconciliation Issues",
                            facts.reconciliation_issues_24h,
                        )
                    ],
                )
            )
        ...
        if failure_clusters:
            top_cluster, top_count = sorted(
                failure_clusters.items(), key=lambda item: item[1], reverse=True
            )[0]
            observations.append(
                self._obs(
        ...
        if stale_runtimes:
            contributors.append(
                PossibleContributor(
                    ...
                    confidence=0.87,
                    time_scope="current platform state",
                    related_observation_codes=[
                        "stale_price_severity",
                        "trade_frequency_shift",
                    ],
                    supporting_metrics=[
                        self._obs_metric(
                            "stale_runtimes", "Stale Runtimes", len(stale_runtimes)
    ...
    def _daily_observations(
        ...
        observations: list[ReviewObservation] = []
        trade_delta = self._delta_value(
            float(facts.trade_count), facts.baseline_trade_count
        )
        pnl_delta = self._delta_value(facts.realised_pnl, facts.baseline_realised_pnl)
        win_rate = (
            (facts.win_count / facts.trade_count) * 100 if facts.trade_count else None
        )
        ...
        if trade_delta is not None and abs(trade_delta) >= 30:
            observations.append(
                self._obs(
                    "trade_frequency_shift",
                    "warning",
                    "Trade frequency shifted",
                    f"Trade count moved {trade_delta:.1f}% versus the rolling daily baseline.",
                    0.82,
                    "review day vs prior 5-day baseline",
                    [
                        self._obs_metric(
                            "trade_count",
                            "Trades",
        ...
        if win_rate_delta is not None and abs(win_rate_delta) >= 20:
            observations.append(
                self._obs(
                    "win_rate_shift",
                    "warning" if win_rate_delta < 0 else "info",
                    "Win rate shifted",
                    f"Win rate moved {win_rate_delta:.1f}% versus the rolling daily baseline.",
                    0.78,
                    "review day vs prior 5-day baseline",
                    [
                        self._obs_metric(
        ...
        if pnl_delta is not None and abs(pnl_delta) >= 25:
            observations.append(
                self._obs(
                    "pnl_delta",
                    "warning" if facts.realised_pnl < 0 else "info",
                    "Realised PnL deviated from baseline",
                    f"Realised PnL moved {pnl_delta:.1f}% versus baseline.",
                    0.76,
                    "review day vs prior 5-day baseline",
                    [
                        self._obs_metric(
        ...
        if failures:
            observations.append(
                self._obs(
                    ...
                    "warning",
                    "Execution failures occurred",
                    f"{len(failures)} failed or manual-review execution events were recorded during the review day.",
                    0.89,
                    "review day",
                    [
                        self._obs_metric(
        ...
        if self._reconciliation_issues(recon_events):
            observations.append(
                self._obs(
                    "reconciliation_issue_pattern",
                    "warning",
                    "Reconciliation issues were present",
                    "Reconciliation events show operational drift during the review day.",
                    0.85,
                    "review day",
                    [
                        self._obs_metric(
    ...
    def _daily_contributors(
        ...
        if risk_rejections:
            contributors.append(
                PossibleContributor(
                    code="risk_rule_pressure",
                    label="Risk rules blocked flow",
                    detail="Risk rejections likely reduced realised trade count.",
                    confidence=0.8,
                    time_scope="review day",
                    related_observation_codes=["trade_frequency_shift"],
                    supporting_metrics=[
                        self._obs_metric(
                            "risk_rejections", "Risk Rejections", facts.risk_rejections
        ...
        if runtime_issues:
            contributors.append(
                PossibleContributor(
                    code="runtime_health_noise",
                    label="Runtime health issues were present",
                    detail="Stale runtime conditions may have reduced normal strategy participation.",
                    confidence=0.77,
                    time_scope="review day",
                    related_observation_codes=["trade_frequency_shift"],
                )
        ...
        if failures:
            contributors.append(
                PossibleContributor(
                    code="execution_friction",
                    label="Execution friction likely affected realised outcomes",
                    detail="Execution failures can reduce fills or create distorted day-level performance.",
                    confidence=0.74,
                    time_scope="review day",
                    related_observation_codes=["execution_failure_cluster"],
                )
    ...
    def _strategy_observations(
        self, facts: StrategyReviewFacts
    ) -> list[ReviewObservation]:
        observations: list[ReviewObservation] = []
        trade_delta = self._delta_value(
            float(facts.trade_count), facts.baseline_trade_count
        ...
        if trade_delta is not None and abs(trade_delta) >= 30:
            observations.append(
                self._obs(
                    ...
                    "warning",
                    "Trade frequency shifted",
                    f"Trade count moved {trade_delta:.1f}% versus the prior baseline window.",
                    0.83,
                    f"last {facts.period_days}d vs previous {facts.period_days}d",
                    [
                        self._obs_metric(
                            "trade_count",
                            "Trades",
        ...
        if win_rate_delta is not None and abs(win_rate_delta) >= 20:
            observations.append(
                self._obs(
                    ...
                    "warning" if win_rate_delta < 0 else "info",
                    "Win rate shifted",
                    f"Win rate moved {win_rate_delta:.1f}% versus the prior baseline window.",
                    0.8,
                    f"last {facts.period_days}d vs previous {facts.period_days}d",
                    [
                        self._obs_metric(
        ...
        if facts.stale_price_events > 0:
            observations.append(
                self._obs(
                    "stale_price_severity",
                    "warning",
                    "Price freshness degraded",
                    f"{facts.stale_price_events} stale runtime event(s) were present for this strategy.",
                    0.92,
                    f"last {facts.period_days}d",
                    [
                        self._obs_metric(
                            "stale_price_events",
                            "Stale Runtime Events",
                            facts.stale_price_events,
                        )
                    ],
                )
            )
        if facts.execution_failures > 0:
            observations.append(
                self._obs(
                    "execution_failure_cluster",
                    "warning",
                    "Execution issues affected the strategy",
                    f"{facts.execution_failures} execution issue(s) were recorded in the selected period.",
                    0.86,
                    f"last {facts.period_days}d",
                    [
                        self._obs_metric(
    ...
    def _strategy_contributors(
        ...
        if stale_runtimes:
            contributors.append(
                PossibleContributor(
                    code="stale_price_pressure",
                    label="Stale pricing may be suppressing normal behaviour",
                    detail="Price freshness degraded for active runtimes tied to the strategy.",
                    confidence=0.85,
                    time_scope=f"last {facts.period_days}d",
                    related_observation_codes=["stale_price_severity"],
                )
            )
        if rejected_intents:
            contributors.append(
                PossibleContributor(
                    code="risk_gating_activity",
                    label="Risk gating blocked candidate entries",
                    detail="Risk rejections may explain some reduction in strategy activity.",
                    confidence=0.81,
                    time_scope=f"last {facts.period_days}d",
                    related_observation_codes=["trade_frequency_shift"],
                )
        ...
        if facts.execution_failures > 0:
            contributors.append(
                PossibleContributor(
                    code="execution_friction",
                    label="Execution issues may have affected realised outcomes",
                    detail="Execution failures can distort both trade count and realised PnL.",
                    confidence=0.76,
                    time_scope=f"last {facts.period_days}d",
                    related_observation_codes=["execution_failure_cluster"],
                )
        ...
        if facts.disconnected_stream:
            observations.append(
                self._obs(
                    ...
                    "The market data stream is enabled but not connected.",
                    0.98,
        ...
        if facts.stale_price_count > 0:
            observations.append(
                self._obs(
                    "stale_price_severity",
                    "critical"
                    if facts.stale_price_count
                    >= max(1, facts.active_runtime_count // 2)
                    else "warning",
                    "Stale prices detected",
                    f"{facts.stale_price_count} runtime(s) are active without fresh prices.",
                    0.95,
                    "current runtime state",
                    [
                        self._obs_metric(
        ...
        if facts.reconciliation_issue_count > 0:
            observations.append(
                self._obs(
                    ...
                    f"{facts.reconciliation_issue_count} reconciliation issue(s) were recorded in the review window.",
                    0.88,
                    "review window",
                    [
                        self._obs_metric(
        ...
        if facts.execution_failure_count > 0:
            observations.append(
                self._obs(
                    "execution_failure_cluster",
                    "warning",
                    "Execution failures recorded",
                    f"{facts.execution_failure_count} execution failure event(s) were recorded in the review window.",
                    0.84,
                    "review window",
                    [
                        self._obs_metric(
    ...
    def _runtime_contributors(
        ...
        if facts.polling_fallback_suspected:
            contributors.append(
                PossibleContributor(
                    ...
                    label="Polling fallback is suspected",
                    detail="Streaming is disconnected while stale counts remain low, which suggests recent polling fallback rather than total data loss.",
                    confidence=0.72,
                    time_scope="review window",
                    related_observation_codes=["stream_disconnect"],
                )
            )
        if facts.heartbeat_issue_count > 0:
            contributors.append(
                PossibleContributor(
                    code="heartbeat_gaps",
                    label="Heartbeat gaps may be contributing to health noise",
                    detail="Missing heartbeats can indicate runtime scheduling or loop issues.",
                    confidence=0.8,
                    time_scope="review window",
                    related_observation_codes=["stale_price_severity"],
                )
    ...
    def _postmortem_observations(
        self, facts: TradePostMortemFacts
    ) -> list[ReviewObservation]:
        observations: list[ReviewObservation] = []
        if facts.outcome == "loss":
            observations.append(
                self._obs(
                    ...
                    f"The selected trade realised {facts.pnl:.2f}.",
                    0.99,
                    "selected trade",
                    [self._obs_metric("pnl", "PnL", facts.pnl, unit="ccy")],
                )
            )
        if facts.execution_warning_count > 0:
            observations.append(
                self._obs(
                    "execution_failure_cluster",
                    "warning",
                    "Execution warnings surrounded the trade",
                    f"{facts.execution_warning_count} execution warning(s) were recorded around the trade lifecycle.",
                    0.83,
                    "selected trade lifecycle",
                    [
                        self._obs_metric(
                            "execution_warning_count",
                            "Execution Warnings",
                            facts.execution_warning_count,
                        )
                    ],
                )
            )
        if facts.recent_loss_count_same_strategy >= 3:
            observations.append(
                self._obs(
                    "loss_cluster",
                    "warning",
                    "Losses cluster around the strategy",
                    f"There are {facts.recent_loss_count_same_strategy} recent losses in the same strategy sample.",
                    0.79,
                    "recent loss sample",
                    [
                        self._obs_metric(
    ...
    def _postmortem_contributors(
        self, trade: Trade, executions: list[Execution], strategy_losses: list[Trade]
    ) -> list[PossibleContributor]:
        contributors: list[PossibleContributor] = []
        if not self._matched_normal_size(trade, strategy_losses[:20]):
            contributors.append(
                PossibleContributor(
                    code="size_outlier",
                    label="Trade size was outside recent norm",
                    detail="The selected trade size differs materially from the recent strategy average.",
                    confidence=0.73,
                    time_scope="selected trade",
                    related_observation_codes=["loss_trade"],
                )
            )
        if any(
            execution.status
            ...
            contributors.append(
                PossibleContributor(
                    code="execution_friction",
                    label="Execution friction was present",
                    detail="Execution issues may have affected the entry or exit quality of the trade.",
                    confidence=0.77,
                    time_scope="selected trade lifecycle",
                    related_observation_codes=["execution_failure_cluster"],
                )
            )
        if len(strategy_losses[:10]) >= 3:
            contributors.append(
                PossibleContributor(
                    code="loss_cluster",
                    label="Trade may belong to a broader losing patch",
                    detail="Several recent losses in the same strategy suggest regime mismatch or timing weakness rather than an isolated outlier.",
                    confidence=0.75,
                    time_scope="recent loss sample",
                    related_observation_codes=["loss_cluster"],
                )
            )
        return contributors

    def _rank_observations(
        self, observations: list[ReviewObservation]
    ) -> list[ReviewObservation]:
        severity_score = {"critical": 3, "warning": 2, "info": 1}
        ordered = sorted(
            observations,
            key=lambda item: (severity_score[item.severity], item.confidence),
            reverse=True,
        )
    ...
    def _previous_fact(
        self, record: GeneratedReviewRecord | None, key: str
    ) -> float | None:
        if record is None:
            return None
    ...
    def _rolling_trade_count_baseline(
        self, now: datetime, *, windows: int, window_days: int
    ) -> float | None:
        counts: list[int] = []
        for offset in range(1, windows + 1):
            ...
            counts.append(
                len(self.trade_service.list_trades(date_from=start, date_to=end))
            )
        return round(mean(counts), 2) if counts else None

    def _rolling_win_rate_baseline(
        self, now: datetime, *, windows: int, window_days: int
    ) -> float | None:
    ...
    def _stale_runtimes(self, runtimes: list[Any], now: datetime) -> list[Any]:
        ...
        return [
            ...
            and (
                ...
                or (
                    now_utc - self._utc_datetime(runtime.last_price_seen_at)
                ).total_seconds()
                > stale_after
            )
        ]
    ...
    def _reconciliation_issues(
        ...
        return [
            event
            for event in events
            if event.event_type != "POSITION_SYNCED_FROM_BROKER"
        ]

    def _reconciliation_pattern_counts(
        self, events: list[ReconciliationEvent]
    ) -> dict[str, int]:
        patterns = Counter(
            event.event_type for event in self._reconciliation_issues(events)
        )
    ...
    def _execution_failure_clusters(
        self, executions: list[Execution]
    ) -> dict[str, int]:
        clusters = Counter(
            f"{execution.strategy_name}:{execution.instrument}"
            for execution in executions
            if execution.status
            in {ExecutionStatus.FAILED.value, ExecutionStatus.NEEDS_MANUAL_REVIEW.value}
        )
    ...
    def _risk_rejection_breakdown(self, intents: list[TradeIntent]) -> dict[str, int]:
        ...
        for intent in intents:
            rule = str(
                intent.decision_reason_code or intent.decision_reason or "unspecified"
            )
            counter[rule] += 1
        return dict(counter)
    ...
    def _execution_warning_messages(self, executions: list[Execution]) -> list[str]:
        ...
        for execution in executions:
            if execution.status in {
                ...
                warnings.append(
                    f"Execution issue: {execution.error_message or execution.reason or execution.status}"
                )
        return warnings
    ...
    def _loss_cluster_patterns(self, losses: list[Trade]) -> list[TradeClusterPattern]:
        ...
        session_counts = Counter(
            self._session_bucket(loss.open_time) for loss in losses
        )
        ...
        return [
            TradeClusterPattern(
                pattern="session_cluster",
                count=top_session_count,
                share_percent=round((top_session_count / len(losses)) * 100, 1),
                detail=f"Losses most often opened during the {top_session} session bucket.",
            ),
            TradeClusterPattern(
                pattern="instrument_cluster",
                count=top_instrument_count,
                share_percent=round((top_instrument_count / len(losses)) * 100, 1),
                detail=f"{top_instrument} is the most common instrument across the sampled losses.",
            ),
        ]

    def _matched_normal_size(
        self, trade: Trade, comparison_trades: list[Trade]
    ) -> bool:
    ...
    def _infer_strategy_name(self, normalized_question: str) -> str | None:
        names = {
            runtime.strategy_name
            for runtime in self.runtime_state_service.list_runtimes()
        } | {trade.strategy_name for trade in self.trade_service.list_trades()[:100]}
    ...
    def _delta_value(
        self, current: float | int | str | None, baseline: float | int | str | None
    ) -> float | None:
    ...
    def _in_period(
        self, timestamp: datetime | None, start: datetime, end: datetime
    ) -> bool:
        return timestamp is not None and self._utc_datetime(
            start
        ) <= self._utc_datetime(timestamp) <= self._utc_datetime(end)

    def _is_since(self, timestamp: datetime | None, since: datetime) -> bool:
        return timestamp is not None and self._utc_datetime(
            timestamp
        ) >= self._utc_datetime(since)
