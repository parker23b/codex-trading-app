...
def get_operator_summary(session: Session = Depends(get_session)) -> OperatorSummaryReview:
...
def get_daily_review(
    ...
    return AIReviewerService(session).get_daily_review(review_date or datetime.now(UTC).date())
...
def get_strategy_review(
    ...
    return AIReviewerService(session).get_strategy_review(strategy_name, period_days=days)
...
@router.get("/trades/{trade_id}/postmortem", response_model=TradePostMortemReviewResponse)
...
def answer_operational_question(
    ...
    return AIReviewerService(session).answer_operational_question(payload.question, strategy_name=payload.strategy_name)
...
def list_review_history(
    ...
    return AIReviewerService(session).list_review_history(review_type=review_type, limit=limit)
