...
class GeneratedReviewRecord(SQLModel, table=True):
    ...
    derived_observations: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
    possible_contributors: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
    warnings: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
    supporting_metrics: list[dict[str, Any]] = Field(default_factory=list, sa_column=Column(JSON))
